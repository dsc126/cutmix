...
from cutmix.cutout import CutOut
from cutmix.mixup import MixUp
...
def main():
    ...
    if args.dataset.startswith('cifar'):
        ...
        if args.dataset == 'cifar100':
            ...
            if args.mode == 'CutMix':
                ...
                train_loader = torch.utils.data.DataLoader(
                    MixUp(ds_train, 100, beta=args.cutmix_beta, prob=args.cutmix_prob, num_mix=args.cutmix_num),
                    batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
            elif args.mode == 'CutOut':
                train_loader = torch.utils.data.DataLoader(
                    CutOut(ds_train, 100, beta=args.cutmix_beta, prob=args.cutmix_prob, num_mix=args.cutmix_num),
                    batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
            else:
                train_loader = torch.utils.data.DataLoader(
                    ds_train,
    ...
    for epoch in range(0, args.epochs):
        ...
        # train for one epoch
        model.train()
        err1, err5, train_loss = run_epoch(train_loader, model, criterion, optimizer, epoch, 'train', args.tensorboard)
        train_err1 = err1
        if args.cv >= 0:
            err1, err5, train_loss = run_epoch(tval_loader, model, criterion, None, epoch, 'train-val', args.tensorboard)
        else:
            pass

        # evaluate on validation set
        model.eval()
        err1, err5, val_loss = run_epoch(val_loader, model, criterion, None, epoch, 'valid', args.tensorboard)
...
def run_epoch(loader, model, criterion, optimizer, epoch, tag, tensorboard):
    ...
    # log to TensorBoard
    if tensorboard:
        log_value(tag + '_loss', losses.avg, epoch)
        log_value(tag + '_error', top1.avg, epoch)
